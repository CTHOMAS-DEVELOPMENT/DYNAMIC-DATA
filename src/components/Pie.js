...
class Pie extends React.Component {
    ...
    render() {
      ...
      let centred={textAlign: "center"}
      let block={displey: "block"}  
      let jsx = getSlices(pie);
      
      return <div style={centred}>
        ...
        <div style={block}>
          <span>&pound;{sent?`${this.formatTextToNumber(sent)} total sent.  `:"" }</span><span>&pound;{remainder?`${this.formatTextToNumber(remainder)} left available.  `:"" }</span>
        </div>
         <hr></hr>
        </div>
    }
